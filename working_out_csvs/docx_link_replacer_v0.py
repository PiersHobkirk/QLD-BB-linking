...
``og_url`` in the CSV, the destination is rewritten to the
corresponding ``remapped_url``.
...
Defaults:
    RAWS_DIR     = ./raws
    MAPPING_CSV  = final_map/proposed_remapped_links_v3.csv
...
def load_mapping(csv_path: str) -> tuple[dict[str, str], list[dict]]:
    ...
    ``url_mapping`` is ``{og_url: remapped_url}``.
        ...
        for row in reader:
            ...
            if old in seen:
                ...
                if prev and prev != new:
                    print(f"  WARNING: duplicate og_url with "
                          f"conflicting remapped_url — keeping first.\n"
                          f"    destination: {old}\n"
                          f"    first:       {prev}\n"
                          f"    duplicate:   {new}")
                continue
...
def main(raws_dir: str, mapping_csv: str, results_csv: str) -> None:
    ...
    print(f"  {len(url_mapping)} unique og_url → remapped_url entries loaded.\n")
